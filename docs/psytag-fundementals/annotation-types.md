...
Psytag provides a variety of annotation types (questions) to support different research designs and data collection needs. These include standard input form elements as well as specialized interactive components designed specifically for behavioral time-series labeling.
...
Annotators can create regions by holding Shift and dragging across the interactive audio/video waveform. Once created, regions can be assigned categorical labels, dragged across the timeline, or resized by dragging their left and right boundary handles.&#x20;
...
* Real-Time Point Dropping: Annotators can drop point markers at the current media playhead position instantly using numerical hotkeys (keys 1 through 9) mapped to pre-defined behavioral categories.
* Manual Marker Placement: Annotators can Shift-click anywhere on the waveform display to place a timestamp marker at a specific timestamp.
* Marker Adjustment: Existing point markers can be dragged horizontally across the waveform to adjust their timing or assigned to new categories.
